...
RSpec.describe Investments::CreatePrice, type: :service do
  context 'with valid attributes' do
    ...
    it 'create price', :aggregate_failures do
      ...
      expect(stock.prices.last.value_cents).to eq((value.to_f * 100).to_i)
    end

    it 'consolidate stock' do
      service

      expect(stock.reload.current_value_cents).to eq((value.to_f * 100).to_i)
      expect(stock.current_total_value_cents).to eq((value.to_f * 100).to_i * stock.shares_total)
    end
