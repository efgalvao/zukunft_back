  ...
  describe 'POST #create' do
    context 'with valid attributes' do
      ...
      it 'create stock', :aggregate_failures do
        ...
        expect(parsed_response['attributes']['invested_value_cents']).to eq((stock_attributes[:invested_value_cents] * 100).to_i)
        expect(parsed_response['attributes']['current_value_cents']).to eq((stock_attributes[:current_value_cents] * 100).to_i)
        expect(parsed_response['attributes']['current_total_value_cents']).to eq((stock_attributes[:current_total_value_cents] * 100).to_i)
